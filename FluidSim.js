class FluidCube {
  constructor(size, diffusion, viscosity, dt, iter) {
    ...
    this.gpgpUtility = new vizit.utility.GPGPUtility(size, size, {premultipliedAlpha: false});
    this.JacobiSolver;
  ...
  Jacobi_lin_solve(b, x, x0, a, c) {
	...
	Jacobi_lin_gpu_solve(b, x, x0, a, c) {
    //console.log("x0");
		//console.log(x0[36]);
		//console.log("x");
		//		console.log(x[36]);
		...
		var N = this.size;
		var cRecip = 1.0 / c;
		//console.log("cRecip" + cRecip);
		var temResult = new Float32Array(this.size * this.size); //copia el array no se si es necesario o deberia arrancar en 0, me parece que es lo mismo.
		for (var k = 0; k < iter; k++) {
			this.JacobiSolver.JacobiIterWrapper(x,x0,a,cRecip,this.size,temResult);
			this.set_bnd(b, temResult);
			copyArray(x, temResult, this.size * this.size);
			//console.log(temResult);
		}
  ...
  Step() {
    ...
    this.project(Vx, Vy, Vx0, Vy0);
		...
		// console.log(s[36]);
		// console.log("density");
		// console.log(density[36]);
    // console.log("Diffuse Start");
...
// 		console.log("density");
// 		console.log(density[36]);
//     console.log("Advect Start");
    this.advect(0, density, s, Vx, Vy);
    //console.log("Advect End");
  }
  ...
  setUpGPUCompute() {
    if (this.gpgpUtility.isFloatingTexture()) {
      ...
      if (bufferStatus.isComplete) {
        ...
        this.JacobiSolver = new JacobiIteration(this.gpgpUtility);
        this.JacobiSolver.init();
